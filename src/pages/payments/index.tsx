  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['user', 'toll', 'transaction.count'],
  ...
  const columns: ColumnType[] = [
    { id: 'amount', header: 'amount', accessorKey: 'amount' },
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.payment_date ? format(record?.payment_date, 'dd-MM-yyyy') : '',
    },
    ...
    hasAccess('toll', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          accessorKey: 'toll',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/tolls/view/${record.toll?.id}`}>
              {record.toll?.name}
            </Link>
          ),
        }
