...
function PaymentViewPage() {
  ...
  const queryParams = useMemo<{ include: { user: boolean; toll: boolean } }>(
    () =>
          ...
          <>
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                ...
                <FormListItem
                  label="Payment Date"
                  text={data?.payment_date ? format(data?.payment_date, 'dd-MM-yyyy') : ''}
                />
                ...
                {hasAccess('toll', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Toll"
                    text={
                      <Link as={NextLink} href={`/tolls/view/${data?.toll?.id}`}>
                        {data?.toll?.name + ''}
                      </Link>
                    }
                  />
