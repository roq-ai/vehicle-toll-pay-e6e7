...
import { useRoqPlatformUserProfiles, useTollCount, useTransactionCount, useVehicleCount } from 'lib/roq';
...
function DashboardPage() {
  ...
  const tollStat = useTollCount();

  const transactionStat = useTransactionCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Tolls',
      result: tollStat?.data,
      isLoading: tollStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Transactions',
      result: transactionStat?.data,
